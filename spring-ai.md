...
3. **PromptTemplate**
   ...
   - `template` is template string with placeholders
   - `params` helps in runtime filling of plaeholder keys with values
4. **Advisor**
...
5. **ChatClient.Builder**
  - ChatOptions: model specific chat options
...
6. ChatMemory
  ...
  - ChatMemoryRepository: binds MessageWindowChatMemory with different storage implemtation like InMemoryChatMemoryRepository, JdbcChatMemoryRepository, Neo4jChatMemoryRepository, CassandraMemoryChatMemoryRepository. 
7. VectorStore
8. ChatClientRequest:
