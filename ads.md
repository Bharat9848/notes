...
##  Actors
 ...
 13. Publisher: More famous the publisher is more easily they get direct contract(reserved ads) after which they sell remanant inventory on an exchange.
----
# Data model
# Campaign
## Campaign types
...
- Set the bids as per suggested bids(by the platform itself) on the lower range. If impressions are not picking up then increase the bids by 1-2 cents.
 ...
 2. **Inventory**
  ...
  - Size of the slot 
 3. Ad
  - Landing Url: Not visible to user on Ad. User go to this page when he click on the Ad.
  - Headline
  - image/text/video
  - URL: visible to user on Ad

----

# Guarnteed Ads/Premium Contracts
 - Cost: CPM/CPTime Bids
----
...
## Contextual: 
 ...
 1. Textual medium
  ...
  - Extract contextual data from web page content using TF-IDF etc.
 2. Multimedia medium
  - Understand the video to match ads, text and discription of the video.
  - Insert ads at specific time in the video so that video's critical message is not lost.
...
- Auction and Bidding process is same as GSP.
- Have charcterisitic of truthfulness.
- Additionally charges bidder the negative externalities to other bidder ???. It is difficult to measure
- have drawback of low seller revenue, vulnerability to collusion and other fradulent behavior.
...
- Along with relevance Ads need to generate revenue for Ad platform. To add revenue into ranking equation in the Ad many methods are introduced.
  1. Quality score in ranking order
  2. lower the relevance
  3. reranking 
  4. substite keyword by finding more matching keywords and high bid amount.
...
- Impression value
 1. Relevancy Score: 
  - For sponsored product Ad/search Ad, how shortlisted SKUs/documents/bidding phrases are relevant to seached keyword, category being browsed, product similar or product complementary category.
  - Technique like query expansion, Ad expansion, user after click behavior feedback are some of the technique that can be used.
...
## Bidding Optimizations
 ...
 - for Click (CPC) It is calculated by `total-ad-spend/ total-clicks`. Low-CPC is good for generic audiance target while high-CPC is also good for narrowed target(high spend users).
 - Click Through Rate
 - Conversion Rate
...
## Cost to Advertiser
- Cost Per Time: Advertiser pay for specified time irrespective the traffic e.g. per day, per month etc.
- Cost Per Milli: It can be fixed(in case of no auction) or dynamic. Advertiser pay for per 1000 impressions. 
- Cost per click
- Cost per action

## Keyword selection
- applicable in case of sponsored Ad.
- bid optimization
- Multi arm bandit for keyword selection
...
## Rough
 ...
 3. The bid for bidder i for a given keyword in the above scenario becomes b0(i) = bid(i)quality-score(i). The payment to the search engine becomes `p(i) = q(i + 1)/q(i)p(i + 1)`. Therefore, if advertisers are able to get a higher quality-based score, they can reduce their cost. However, one should note that ADX also use quality-based scores to set limitations, meaning that advertisers may have to bid more to get in the competition, even if there is no competitor. For example, if the minimum allowed quality score × bid is 10 in Google AdWords, company A now has to bid at least $1 × 10 8 = $1.25 for their ad to be displayed. from paper - Internet Advertising: An Interplay among Advertisers, Online Publishers, Ad Exchanges and Web Users 
 4.
